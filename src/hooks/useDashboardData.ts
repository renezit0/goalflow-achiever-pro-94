  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        // Buscar vendas até ontem (para cálculo da meta diária) - 2025-09-01
        const ontem = new Date('2025-09-01');
        const ontemStr = '2025-09-01';
        console.log('📅 Data ontem CORRIGIDA:', ontemStr);
        ...
        // Buscar vendas do dia atual (2025-09-02 é considerado "hoje")
        const hoje = new Date('2025-09-02');
        const hojeStr = '2025-09-02';
        console.log('📅 Data hoje CORRIGIDA:', hojeStr);
        ...
        console.log('🎯 Vendas de HOJE encontradas:', vendasHoje?.length, 'dados:', vendasHoje, errorVendasHoje ? 'ERRO:' + errorVendasHoje.message : '');
        ...
        for (const categoria of categorias) {
          ...
          if (categoria.id === 'r_mais') {
           ...
           console.log(`💼 ${categoria.name}: Hoje=${totalVendidoHoje.toFixed(2)} (${vendasCategoriaHoje.length} vendas), Período=${totalVendidoPeriodo.toFixed(2)} (${vendasCategoria.length} vendas), Meta=${metaValor.toFixed(2)}`);
