...
export default function Login() {
  ...
  return (
    <div className="login-container min-h-screen relative overflow-hidden" style={{ background: 'var(--bg-color)' }}>
      {/* Animated Rings */}
      <div className="ring">
        <i></i>
        <i></i>
      ...
      <div className="download-section fixed bottom-5 right-5 z-50">
        <button 
          className="download-toggle group relative w-14 h-14 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 animate-float"
          style={{ 
            background: 'linear-gradient(45deg, var(--ring-color-2), var(--ring-color-1))'
          }}
        >
          <i className="fas fa-download group-hover:scale-110 transition-transform duration-300" style={{ color: 'var(--text-on-secondary)' }}></i>
          <div 
            className="new-indicator absolute -top-1 -right-1 w-3 h-3 rounded-full animate-pulse"
